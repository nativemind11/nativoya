...
## تحديث: عنوان لكل سامبل صوتي

```sql
-- شغّل db/migration_audio_sample_titles.sql
```
ضاف عمود `audio_sample_titles` (مصفوفة نصوص) لجدول `tasks`، بيتوافق مع `audio_sample_urls`
عنصر بعنصر حسب الترتيب. لما الهيد ليدر يضيف تاسك أو يعدّله ويحط أكتر من سامبل، كل واحد
بقى ليه عنوان خاص بيه بيظهر فوق مشغّل الصوت في صفحة المهمة.
